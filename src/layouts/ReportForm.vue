<template>
  <custom-modal class="text-left" @close-modal="this.$emit('close-form')">
    <div>
      ...
      >
        <option v-for="reasonTemp in this.reasons" :key="reasonTemp.id" :value="reasonTemp.id">
          {{ reasonTemp.reason }}
        </option>
      </select>
    </div>
...
export default {
  ...
  methods: {
    async getReasons(){
      this.reasons = (await Report.getReportReasons(sessionStorage.getItem("token"))).reasons;
    },
    async submitReport(){
