<template>
  ...
  >
    <div class="w-full h-auto mx-auto px-4 flex gap-5 items-center">
      ...
      <div>
        <p
          class="text-black bold mb-0 uppercase cursor-pointer"
          @click="this.$router.push('/profile/' + rating.rater.id)"
        >
    ...
    <div class="bg-none rounded-full flex px-20 mt-2">
...
export default {
  ...
  methods: {
    calculateStarColor(starNumber) {
      return this.rating.stars_number && starNumber <= this.rating.stars_number
        ? this.starsColor
        : "none";
    },
    raterImage() {
      return this.rating.rater.profile_picture
        ? `${this.storageLink}/${this.rating.rater.profile_picture}`
